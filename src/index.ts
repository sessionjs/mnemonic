...
export function encode(
	str: string,
	wordsetName: MnemonicWordset = MnemonicWordset.english,
): string {
	...
	for (let j = 0; j < strCopy.length; j += 8) {
		strCopy = strCopy.slice(0, j) + swapEndian4byte(strCopy.slice(j, j + 8)) + strCopy.slice(j + 8);
	}
...
function swapEndian4byte(str: string) {
	if (str.length !== 8) {
		throw new SessionValidationError({
			...
			message: `Invalid input length: ${str.length}`,
		});
	}
...
export function decode(
	...
	if (wlist.length < 12) {
		throw new SessionValidationError({
			code: SessionValidationErrorCode.InvalidMnemonic,
			message:
				"Invalid number of words in provided string when decoding mnemonic. Expected 13 words, got less than that",
		});
	}
	if (
	...
	if (wordset.prefixLen > 0 && wlist.length % 3 === 0) {
		throw new SessionValidationError({
			code: SessionValidationErrorCode.InvalidMnemonic,
			message:
				"Invalid number of words in provided string when decoding mnemonic. Missing last checksum word",
		});
	}
	if (wordset.prefixLen > 0) {
	...
	for (let i = 0; i < wlist.length; i += 3) {
		...
		if (w1 === -1 || w2 === -1 || w3 === -1) {
			throw new SessionValidationError({
				code: SessionValidationErrorCode.InvalidMnemonic,
				message: "Invalid word in mnemonic",
			});
		}
		...
		if (x % n !== w1) {
			throw new SessionValidationError({
				code: SessionValidationErrorCode.InvalidMnemonic,
				message: "Couldn't decode mnemonic",
			});
		}
	...
	if (wordset.prefixLen > 0) {
		...
		if (
			...
			throw new SessionValidationError({
				code: SessionValidationErrorCode.InvalidMnemonic,
				message: "Invalid checksum last word in mnemonic",
			});
		}
	}
	return out;
...
export const addMnemonicLanguage = ({
	prefixLen,
	words,
}: {
	prefixLen: number;
...
export const mnemonicLanguages: {
	[key in MnemonicWordset]: { prefixLen: number; words: string[]; truncWords: string[] };
} = {
